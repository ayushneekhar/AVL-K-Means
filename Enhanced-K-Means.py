...
class cluster:
    ...
    def removepoint(self, pointid):                 #To Remove a Point from a Cluster
        self.clusterList.remove(pointid)

    def addpoint(self, pointid):                    #To add a point in a Cluster
        self.clusterList.append(pointid)

    def get_centroid(self, points):                 #To Calculate the Centroid
...
class Kmeans:
    ...
    def Euclidian_distance(self, p1, center):           #Function to Fined Euclidian distance of a Point from a Cneteroid
    ...
    def run(self):
        if self.K>len(self.points):
            return
        self.points = np.array(self.points).astype(float)           #Returns all points as an array of floats
        np.random.seed(42)
        random_index = np.random.choice(range(len(self.points)), self.K, replace=False)     #Choose a Random Centeroid
        ...
        for i in range(len(self.points)):
            ...
            for j in range(self.K):
                distance_to_centroid[i][j] = self.Euclidian_distance(self.points[i],centroids[j])       #Finds Distance of a Point to Centeroid
...
image_seg_orig = cv2.imread("Image.jpg")        #reading the image
cap = cv2.resize(image_seg_orig, (256,256))     #rezizing the image
cap = cv2.cvtColor(cap,cv2.COLOR_BGR2RGB)       #converting to a more optimal colour space
h,w,z = cap.shape
textFileList = []
biglist=[]
...
obj = Kmeans(100,4)                                         #Initializing the K means algorithm with 400 iterations and 2 clusters
